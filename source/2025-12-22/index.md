...
5. 調節を行い、okの場合は Apply as Rest PoseとしてAポーズを適用します

![alt text](apose.png)

### Bodyと衣装の分割エクスポート
Aポーズ同様に、メッシュを選択、Body・衣装のスロット別々に分けて登録していきます。
その後、Export Separeted Partsを実行すると、アバターはFBX、衣装はobjとしてエクスポートします。
![alt text](body.png)
![alt text](export.png)

###　マテリアルの一括削除
マテリアルエラーになっているマテリアルがあると、Marvelous Designerにうまく読み込めな苦なるため必要に応じて削除します。
